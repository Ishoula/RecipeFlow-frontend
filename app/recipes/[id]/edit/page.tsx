...
export default function EditRecipePage() {
  ...
  const loadRecipeDetails = async () => {
    try {
      ...
      // Ownership check
      if (recipeData.user && recipeData.user.id !== user?.id) {
        setIsOwner(false);
        setLoading(false);
        return;
      }
      ...
      setFormData({
        ...
        equipments: recipeData.equipments || [],
      ...
      setErrors({ load: 'Failed to load recipe details. It may not exist.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await recipeAPI.updateRecipe(parseInt(recipeId), formData);
      router.push('/recipes/my');
    } catch (error: any) {
      setErrors({
        submit: error.response?.data?.message || 'Failed to update recipe',
      });
    } finally {
      setSaving(false);
    }
          ...
          <div className="max-w-md mx-auto px-4 py-24 text-center">
            <div className="bg-white rounded-xl shadow-lg p-8 border border-red-100">
              ...
              <p className="text-gray-600 mb-6 leading-relaxed">
                You do not have permission to edit this recipe. Only the author of this recipe can edit it.
              </p>
              <Link
                href="/recipes/my"
                className="px-6 py-2.5 bg-primary text-white font-bold rounded-lg hover:brightness-95 transition inline-flex items-center gap-2 shadow-sm"
              >
                Go to My Recipes
              </Link>
            </div>
        ...
        <div className="min-h-screen bg-light">
          <div className="max-w-full sm:max-w-lg lg:max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
            ...
            <div className="bg-white rounded-lg shadow-lg p-8">
              <h1 className="text-3xl font-bold text-dark mb-8">Edit Recipe</h1>
